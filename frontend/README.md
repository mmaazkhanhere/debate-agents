...
It is a Next.js App Router frontend for running and visualizing AI powered multi agent debates in real time.
This UI connects to the Debate Backend to orchestrate debates between simulated personas and stream round-by-round results live in the browser.

## What This App Does
Users can:
- Select two AI debaters
- Choose a debate topic
- Start a real time debate session
- Watch sequential rounds unfold live via SSE
- View structured judge panel scoring
- Review analytics (token usage, cost, duration, outcomes)
...
- Structured event mapping (agents, presenter, judges)
- Browser-based session persistence
- Fully Dockerized dev & production workflows
- Smooth motion transitions (Framer Motion)
- Card based debate visualization (attack / defense / refute move types)
...
| Route                 | Purpose                        |
| --------------------- | ------------------------------ |
| `/`                   | Debate setup and configuration |
| `/debate/[debate_id]` | Live debate arena              |
| `/analytics`          | Usage and debate history       |
...
This UI renders AI-generated debates between simulated personas.

Outputs:
- Are generated by large language models
- May contain inaccuracies or bias
- Do not represent real statements from any public figure
- Are for demonstration and educational purposes only

The project is not affiliated with or endorsed by any real individuals represented as personas.
